...
#ifndef __CHLOE_MATRIX_H__
#define __CHLOE_MATRIX_H__

#include "CLVector.h"
#include "CLMatrixQuaternion.h"
...
namespace CL
{
	...
	Matrix Matrix::WithTranslation(const Vector3 &translation)
	{
		Matrix mat;
	...
	Matrix Matrix::WithTranslation(const Vector4 &translation)
	{
		Matrix mat;
	...
	Matrix Matrix::WithScaling(const Vector3 &scaling)
	{
		Matrix mat;
	...
	Matrix Matrix::WithScaling(const Vector4 &scaling)
	{
		Matrix mat;
	...
	Matrix Matrix::WithRotation(const Vector3 &rotation)
	{
		Quaternion quat(rotation);
		return quat.GetRotationMatrix();
	}
	
	Matrix Matrix::WithRotation(const Vector4 &rotation)
	{
		Quaternion quat(rotation);
		return quat.GetRotationMatrix();
	}
	
	Matrix Matrix::WithRotation(const Quaternion &rotation)
	{
		Quaternion quat(rotation);
		return quat.GetRotationMatrix();
	}
	...
	Matrix Matrix::WithProjectionOrthogonal(float left, float right, float bottom, float top, float clipnear, float clipfar)
	{
		Matrix mat;
	...
	Matrix Matrix::WithProjectionPerspective(float arc, float aspect, float clipnear, float clipfar)
	{
		Matrix mat;
	...
	Matrix Matrix::WithInverseProjectionPerspective(float arc, float aspect, float clipnear, float clipfar)
	{
		Matrix mat;
	...
	float Matrix::GetSubmatrixDeterminant(const int k) const
	{
		int i = k%4; // i <=> j
		int j = k/4;
		int i1 = (i == 0);
	...
	Vector3 Matrix::GetEulerAngle() const
	{
		Vector3 result;
	...
	Vector4 Matrix::GetAxisAngle() const
	{
		return GetQuaternion().GetAxisAngle();
	}
	
	Quaternion Matrix::GetQuaternion() const
	{
		Quaternion result;
	...
	void Matrix::Translate(const Vector4 &translation)
	{
#if RN_SIMD
		SIMD::VecFloat result = SIMD::Mul(vec[0], SIMD::Set(translation.x));
		result = SIMD::Add(result, SIMD::Mul(vec[1], SIMD::Set(translation.y)));
	...
	void Matrix::Scale(const Vector3 &scaling)
	{
		*this *= Matrix::WithScaling(scaling);
	}
	
	void Matrix::Scale(const Vector4 &scaling)
	{
		*this *= Matrix::WithScaling(scaling);
	}
	...
	void Matrix::Rotate(const Vector4 &rotation)
	{
		*this *= Matrix::WithRotation(rotation);
	}
