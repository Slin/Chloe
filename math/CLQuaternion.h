...
#ifndef __CHLOE_QUATERNION_H__
#define __CHLOE_QUATERNION_H__
...
{	
	...
	Quaternion::Quaternion(const Vector4 &axis)
	{
	...
	Quaternion Quaternion::WithIdentity()
	{
		return Quaternion();
	}
	
	Quaternion Quaternion::WithEulerAngle(const Vector3 &euler)
	{
		Quaternion temp;
	...
	Quaternion Quaternion::WithAxisAngle(const Vector4 &axis)
	{
		Quaternion temp;
	...
	Quaternion Quaternion::WithLerpSpherical(const Quaternion &start, const Quaternion &end, float factor)
	{
	...
	Quaternion Quaternion::WithLerpLinear(const Quaternion &start, const Quaternion &end, float factor)
	{
		float inverseFactor = 1.0f - factor;
		return (end * factor) + (start * inverseFactor);
	}
	
	Quaternion Quaternion::WithLookAt(const Vector3 &tdir, const Vector3 &tup, bool forceup)
	{
		Quaternion temp;
	...
	Quaternion Quaternion::GetLerpSpherical(const Quaternion &other, float factor) const
	{
		return WithLerpSpherical(*this, other, factor);
	}
	
	Quaternion Quaternion::GetLerpLinear(const Quaternion &other, float factor) const
	{
	...
	Vector4 Quaternion::GetRotatedVector(const Vector4 &vector) const
	{
		Quaternion vectorquat(vector.x, vector.y, vector.z, 0.0f);
		Quaternion resultquat = (*this) * vectorquat * GetConjugated();
	...
	float Quaternion::GetDotProduct(const Quaternion &other) const
	{
